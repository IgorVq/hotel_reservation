...
function findClientsByBookingYear(year){
    return connection.promise().query('select * from client inner join reservation on reservation.id_client = client.id_client where year(check_in_date) = ? ', [year]).then((results) => {
        return results[0];
    });
}

function findClientsByMinCostBooking(amount){
    return connection.promise().query('select * from client inner join reservation on reservation.id_client = client.id_client where total_cost >= ? ', [amount]).then((results) => {
        return results[0];
    });
}
...
function findClientsByCostBooking(min, max){
    return connection.promise().query('select * from client inner join reservation on reservation.id_client = client.id_client where total_cost >= ? and total_cost <= ?', [min, max]).then((results) => {
        return results[0];
    });
}
...
function findClientsByBookingYearMonth(year, month){
    return connection.promise().query('select * from client inner join reservation on reservation.id_client = client.id_client where year(check_in_date) = ? and month(check_in_date) = ?', [year, month]).then((results) => {
        return results[0];
    });
}
...
function findClientByTopCostBooking(){
    return connection.promise().query('select client.id_client, first_name, last_name, email, phone, registration_date, payment.amount from client inner join reservation on reservation.id_client = client.id_client inner join payer on payer.id_reservation = reservation.id_reservation inner join payment on payment.id_payment = payer.id_payment order by amount desc limit 1').then((results) => {
        return results[0];
    });
}
